...
El presente tutorial contiene preguntas relacionadas con la estimación del modelo y sus supuestos, conceptos importante para la compresión los proceso relacionados con la inferencia realizada sobre los resultados obtenidos por el método de MCO.   
...
El intervalo de confianza para $\beta_{i}$ está dado por :

$$
\widehat\beta_j \pm t_{\alpha/2, n - p}\, \text{se}\left(\widehat\beta_j\right)
$$

y el estadístico de prueba para $Ho: \beta_{j} = B_{0}$ vs $Ho: \beta_{j} \neq B_{0}$ es
...
| concepto                   | código                                             |
|:---------------------------|:---------------------------------------------------|
| estimación MCO             |`lm(y ~ x1 + x2 + x3 , data = namedata)`            |
...
A continuación se presenta los resultados de la estimación MCO el modelo1 : 
...
quiz(
  question("",
           ...
           allow_retry = TRUE,
           answer("Un parqueadero adicional, incrementa el precio de la vivienda en 130.6 millones ", message = "No...inténtalo de nuevo!"),
           ...
           answer("La razón por la que el coeficiente tenga un valor negativo, es que una vivenda por lo menos tiene un baño y una habitación y su área construida es superior a 30 m2", message = "No...inténtalo de nuevo!"),
...
quiz(
  question("¿Pregunta?",
           correct = "Correcto!",
           allow_retry = TRUE,
           answer("El valor del estadístico F, indica que el modelo no es adecuado para la estimación del precio", message = "Cerca, pero no...inténtalo de nuevo!"),
           answer("El valor de R2 indica que el modelo es apropiado para la explicación de la variabilidad del precio", message = "No...inténtalo de nuevo!"),
           answer("El valor-p correspondeinte a la prueba de significancia individual para la variable regresora areaconst indica que su valor no es significativo estadísticamente", message = "Incorrecto. Intenta de nuevo!."),
...
quiz(
  question("",
           ...
           allow_retry = TRUE,
           answer("Para poder que el método de MCO encuentre una solución única ", message = "No...inténtalo de nuevo!"),
           answer("Poder encontrar el valor de la variable respuesta,  dado un conjunto de valores de las variables regresoras", message = "No...inténtalo de nuevo!"),
...
quiz(
  question(" ",
           ...
           allow_retry = TRUE,
           answer("Todas las casas tienen un mayor valor que todos los apartamento", message = "Cerca, pero no...inténtalo de nuevo!"),
           answer("Todas las casas tienen un inferior  que todos los apartamentos", message = "Nop...inténtalo de nuevo!"),
...
<!--            allow_retry = TRUE, -->
<!--            answer("opción 1", message = "Cerca, pero no...inténtalo de nuevo!"), -->
<!--            answer("opción 2", message = "Nop...inténtalo de nuevo!"), -->
